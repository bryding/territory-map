...
describe('CSV Parser End-to-End Tests', () => {
  it('should parse actual sample-data.csv file and extract notes correctly', async () => {
    ...
    const customersWithNotes = result.data.filter(
      (customer) => customer.notes.general || customer.notes.contact || customer.notes.product,
    )

    console.log(
      `📝 Found ${customersWithNotes.length} customers with notes out of ${result.data.length} total`,
    )
