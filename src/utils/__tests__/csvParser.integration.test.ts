...
Kaiti Green,Castle Rock Clinic (CN666666),8642 Castle Pines Dr Castle Rock 80108,Castle Rock,Dr. Castle,,,,SKINPEN,\"$1,000\"`
      ...
      expect(customers.find((c) => c.customerNumber === 'CN111111')?.territory).toBe(
        'colorado-springs-north',
      )
      expect(customers.find((c) => c.customerNumber === 'CN222222')?.territory).toBe(
        'colorado-springs-central',
      )
      expect(customers.find((c) => c.customerNumber === 'CN333333')?.territory).toBe(
        'colorado-springs-south',
      )
      expect(customers.find((c) => c.customerNumber === 'CN444444')?.territory).toBe(
        'highlands-ranch',
      )
...
Kaiti Green,Multi Brand (CN123456),123 Test St,Test City,Dr. Test,,,,SKINPEN,\"$300\",\"$400\"`
      ...
      const totalDaxxify = Object.values(customer.salesData.daxxify.salesByPeriod).reduce(
        (a, b) => a + b,
        0,
      )
      const totalRha = Object.values(customer.salesData.rha.salesByPeriod).reduce(
        (a, b) => a + b,
        0,
      )
      const totalSkinPen = Object.values(customer.salesData.skinPen.salesByPeriod).reduce(
        (a, b) => a + b,
        0,
      )
